...
public class QasmImportStrategy implements ImportStrategy {

    @Override
    public QuantumCircuit importCircuit(String filename) {
        try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
            String content = readFileContent(reader);
            QuantumCircuit circuit = parseCircuitFromQasm(content);
            System.out.println("Circuit imported from QASM: " + filename);
            return circuit;
        } catch (IOException e) {
            throw new RuntimeException("Error importing from QASM: " + e.getMessage(), e);
        }
    }

    @Override
    public UndoableCommand createImportCommand(QuantumCircuit targetCircuit, String filename) {
        return new ImportQASMCommand(targetCircuit, filename);
    }
    ...
    private QuantumCircuit parseCircuitFromQasm(String qasmContent) {
        QasmParser parser = new QasmParser(qasmContent);
        return parser.parse();
    }
}
