...
import control.command.gate.AddGateCommand;
import control.command.gate.GateType;
import control.command.gate.RemoveGateCommand;
import control.command.circuit.AddQubitCommand;
import control.command.circuit.RemoveQubitCommand;
import control.command.simulation.SimulateCommand;
import control.export.CircuitExporter;
import control.export.ExportFormat;
...
public class Controller {
    ...
    public QuantumCircuitController(int numQubits) {
    ...
    public void addHadamard(int qubit) {
        addGate("hadamard", qubit);
    }

    public void addPauliX(int qubit) {
        addGate("pauli-x", qubit);
    }

    public void addPauliY(int qubit) {
        addGate("pauli-y", qubit);
    }

    public void addPauliZ(int qubit) {
        addGate("pauli-z", qubit);
    }

    public void addTGate(int qubit) {
        addGate("t", qubit);
    }

    public void addSGate(int qubit) {
        addGate("s", qubit);
    }
    ...
    public void addControlledGate(QuantumGate gate, int control, int... targets) {
        circuit.addControlled(gate, control, targets);
    }

    // ============================================================================
    // CIRCUIT STRUCTURE OPERATIONS
    // ============================================================================
    ...
    public void addQubit() {
        ...
        // Update references after command execution
    ...
    public void setQubitCount(int targetQubits) {
        ...
        if (targetQubits > current) {
            addQubits(targetQubits - current);
        } else if (targetQubits < current) {
            for (int i = current; i > targetQubits; i--) {
                removeQubit();
            }
        }
    }
    ...
    public void executeCircuit() {
        circuit.executeOn(currentState);
    }
    ...
    public String getLastCommandType() {
        return commandHistory.getLastCommandType();
    }

    public int getHistorySize() {
        return commandHistory.getUndoStackSize();
    }
    ...
    public void exportCircuit(ExportFormat format) {
        exporter.export(circuit, format);
    }

    public void exportToJSON() {
        exportCircuit(ExportFormat.JSON);
    }

    public void exportToQASM() {
        exportCircuit(ExportFormat.QASM);
    }
