...
public class QuantumState {
    ...
    public QuantumState(Complex[] amplitudes, int numQubits) {
        ...
        if (amplitudes.length != expectedSize){throw new IllegalArgumentException("Incorrect number of amplitudes");}
    ...
    public QuantumState applyGate(QuantumGate gate) {
        if (gate.getNumQubits() != this.numQubits){throw new IllegalArgumentException("Gate requires " + gate.getNumQubits() + "qubits");}
    ...
    private void collapse(int measuredState) {
        if (measuredState < 0 || measuredState >= amplitudes.length) {throw new IllegalArgumentException("Invalid measured state: " + measuredState);}
        for (int i = 0; i < amplitudes.length; i++) {
            amplitudes[i] = (i == measuredState) ? new Complex(1.0, 0.0) : new Complex(0.0, 0.0);
        }
    }
    ...
    public double[] getProbabilities() {
        ...
        for (int i = 0; i < amplitudes.length; i++) {probabilities[i] = amplitudes[i].magnitudeSquared();}
        return probabilities;
    }

    private void validateStateNormalization() {
        if (!utils.isNormalized(amplitudes, this)) {throw new IllegalStateException("Cannot measure non-normalized quantum state");}
    }
    ...
    public QuantumState applyPhaseFlip(int targetQubit) {
        ...
        for (int i = 0; i < amplitudes.length; i++) {
            ...
            if (targetQubitIsOne) {newAmplitudes[i] = amplitudes[i].multiply(new Complex(-1.0, 0.0));
            } else {newAmplitudes[i] = amplitudes[i];}
        }
        return new QuantumState(newAmplitudes, this.numQubits);
    }

    public QuantumState normalizeState(Complex[] amplitudes) {
        double norm = 0.0;
        for (Complex amp : amplitudes) {norm += amp.magnitudeSquared();}
        if (norm == 0.0) {throw new IllegalStateException("Cannot normalize zero state");}
